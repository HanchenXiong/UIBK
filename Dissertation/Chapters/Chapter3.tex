...
In the past two decades, due to increasingly richer structures in modern data or learning tasks, the research of (supervised) machine learning has been evolving from simple binary 
classification, or scaler-output 
regression, to structured-output learning \citep{Bakir07}. Naturally, graphs can be constructed to encode the interesting interactions among the variables within complex output structures, 
which leads to the 
popular use of (probabilistic) graphical models \citep{LGM}. Conventionally, a graphical model is specifically designed to match the scenario of a given application by providing a joint probability of all 
involved variables. For instance, in a multi-label learning case, input $\mathbf{x}\in{\mathbb{R}^D}$ is a $D$-dimensional real-value vector, output $\mathbf{y}\in\mathbb{B}^L$ is a $L$-dimensional 
